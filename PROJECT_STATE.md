...
- **Configurar el token (obligatorio, una vez)**: el backend exige un header `X-SimplyApply-Token` en los endpoints que usa la extensión (protección añadida tras auditoría de seguridad — ver abajo). No se guarda aquí (es un secreto local, no algo para versionar). Pegar en el icono de la extensión → Options. **Ojo**: si algún día se borra `backend/data/simplyapply.db` o se arranca el backend por primera vez en otra máquina, se genera un token nuevo — leerlo del log de arranque de uvicorn o con la consulta de abajo:
  ```bash
